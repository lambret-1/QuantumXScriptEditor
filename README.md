...
- 修复静态检查功能视图弹出后不能自动回调的bug：
  ...
  - 【修复2】视图模型新增`清除检查结果()`方法，用于关闭检查结果视图时清除@Published的检查结果
  - 【修复3】静态检查结果视图新增`关闭回调`闭包属性，支持外部传入关闭时的回调逻辑
  - 修改文件：Sources/View/脚本编辑器页.swift、Sources/ViewModel/脚本编辑器视图模型.swift
...
- 优化JSON广告屏蔽模板：父子路径去重
  ...
  - 【日志增强】头部注释和日志显示原始识别字段数、去重后字段数、合并字段数，方便用户了解去重效果
  - 例如：识别到 data.globalData、data.globalData.pupu、data.globalData.appver 三个字段，去重后只清空 data.globalData = {}
  - 修改文件：Sources/ViewModel/脚本测试视图模型.swift

### v1.7.8
- 大幅扩充广告分析关键词库：
  - 【广告标记关键词】从90+扩充到200+：新增信息流/时间线广告、贴片广告(前贴/中贴/后贴)、角标/徽章广告、通知/推送广告、广告策略/规则/版本、会员免广告、广告计数/频率/间隔、广告优先级/权重、广告调试/测试、更多广告配置对象(SDK/网络/平台/供应商/代理商/合作伙伴)、更多广告类型(富媒体/可扩展/粘性/广告墙/OfferWall/可玩广告/内流/外流)
  - 【广告资源关键词】从150+扩充到500+：新增更多广告图片资源(图片URL/封面/缩略图/各类型广告图片)、更多广告视频资源(视频链接/源/路径/MP4/M3U8/视频参数)、更多广告音频资源(音频/语音/音乐/音效/MP3)、更多广告HTML/富媒体资源(HTML/iframe/embed/代码片段/脚本/CSS/JS)、更多广告位置/槽位(槽位ID/位置ID/放置/区域/空间/位置/点位/单元/标签/分类)、更多广告样式/模板(样式/模板/布局/主题/颜色/尺寸/圆角/边框/内边距/外边距/字体/动画/效果/过渡)、更多广告交互/按钮(关闭/跳过/静音/全屏/播放/暂停/停止/重播/下载/安装/分享/收藏/点赞/评论)、更多广告落地页/跳转(页面URL/网页URL/站点URL/H5/小程序/快应用/Scheme/Intent/通用链接/AppLink/深链/目标URL/最终URL/落地页/点击URL)、更多广告品牌/广告主(品牌URL/Logo/图标/图片/广告主ID/URL/Logo/图标/所有者/赞助商/代理商/合作伙伴/供应商/提供商/网络/平台)、更多广告统计/上报(回调/通知/Ping/Beacon/日志/事件/各类型事件(曝光/展示/查看/点击/关闭/跳过/完成/奖励/开始/结束/播放/暂停/静音/全屏))、更多广告价格/积分/预算(金额/成本/预算/出价/CPM/CPC/CPA/CTR/CVR/ROI/eCPM/收入/利润/花费/费用/价格/金币/积分/奖励)
  - 【广告数组关键词】从60+扩充到300+：新增更多按类型分类的广告列表(各类型广告的List/Array)、更多广告位置/槽位列表、更多广告资源列表(图片/视频/音频/HTML/封面/缩略图/图标/Logo)、更多轮播/幻灯片列表、更多信息流/时间线列表(feed/stream/timeline/content/article/post/news/story/moment/dynamic)、更多推荐/热门/排行榜列表(推荐/热门/Top/排名/排行榜/趋势/流行/精选/编辑推荐)、更多弹窗/通知/消息列表(通知/消息/弹窗/对话框/Toast/提示/引导/教程)、更多活动/促销/营销列表(活动/战役/事件/促销/特卖/折扣/优惠券/代金券/礼品/奖品/奖励/奖金/抽奖/任务/使命/签到)、更多品牌/广告主列表、更多广告配置/策略列表、更多广告统计/报表列表
  - 修改文件：Sources/Service/智能分析服务.swift

### v1.7.7
- 两个广告模板增加详细日志输出：
  - 【JSON模板】关键步骤增加打印：脚本触发→获取响应体→确认JSON→JSON解析成功→开始屏蔽→每个字段屏蔽完成（带进度X/总数）→结尾打印"共清空X个广告字段"
  - 【非JSON文本替换模板】关键步骤增加打印：脚本触发→获取响应体→开始检测关键词→未命中直接放行→命中开始清除→每个关键词清除完成（带进度X/总数）→结尾打印"共清空X个广告关键词"
  - 修改文件：Sources/ViewModel/脚本测试视图模型.swift

### v1.7.6
- 修复广告JSON模板测试不通过的bug：
  - 【修复1】根因：globalData/appver等对象类型字段被错误归类为广告标记类型，导致生成脚本时被设为0而非{}。修复：根据字段当前值格式智能判断空值类型（以{开头设为{}，以[开头设为[]，以"开头设为""，布尔/数字标记设为0），不再仅依赖字段类型
  - 【修复2】支持顶层广告字段（路径只有一级，如body.ad），之前被guard跳过不生成修改代码
  - 修改文件：Sources/ViewModel/脚本测试视图模型.swift

### v1.7.5
- 测试面板体验优化：
  - 【优化1】移除测试面板独立背景色，测试面板融入页面，跟随全局ScrollView一起滑动，不再有画中画感
  - 【优化2】输出区域自动滚动默认关闭，测试结束后不自动滚到底部，用户可手动开启自动滚动开关
  - 修改文件：Sources/View/脚本测试面板.swift、Sources/ViewModel/脚本测试视图模型.swift

### v1.7.4
- 广告分析功能增强：支持非JSON格式响应体的广告屏蔽
  - 【增强1】自动识别响应体类型：JSON格式修改字段值，非JSON格式（HTML/JS/文本）用关键词检测+正则替换
  - 【增强2】非JSON分析时搜索广告关键词在文本中的出现，去重后生成关键词列表
  - 【增强3】生成文本替换式广告屏蔽脚本：快速预判（不含关键词直接返回）→命中后执行全局替换，参考用户提供的实战脚本风格
  - 【增强4】广告分析弹窗显示响应体类型标签（JSON/文本），非JSON时用标签云展示识别到的广告关键词
  - 【增强5】新增换行布局视图（LazyVGrid实现），用于关键词标签云展示
  - 修改文件：Sources/Service/智能分析服务.swift、Sources/ViewModel/脚本测试视图模型.swift、Sources/View/广告分析弹窗.swift

### v1.7.3
- 优化广告分析一键生成模板，大幅精简生成代码：
  - 【优化1】父路径存在性检查去重：之前每个字段都重复生成完整的父路径检查代码，现在相同父路径只生成一次，按路径长度排序确保先外层后内层，字段越多节省越明显
  - 【优化2】精简日志：移除每个字段的独立console.log，只保留一条汇总日志，减少日志输出量
  - 【优化3】精简注释和结构：移除冗余的步骤编号注释、移除额外的函数包装`执行广告屏蔽`，直接在try块中修改，代码结构更紧凑
  - 【优化4】简化Content-Type检查逻辑，合并条件判断
  - 修改文件：Sources/ViewModel/脚本测试视图模型.swift

### v1.7.2
- 彻底修复编辑器输入抖动问题（终极方案）：
  - 【根因】之前的防抖高亮方案（停止输入0.3秒后高亮）在高亮执行时仍会重新设置attributedText，导致突然的延迟抖动，用户体验更差
  - 【终极修复】输入过程中完全不触碰attributedText，只更新绑定文本、刷新行号、更新补全候选，输入体验与原生UITextView完全一致，零抖动
  - 【高亮时机】语法高亮只在结束编辑（resignFirstResponder）时应用一次，保存选中范围后恢复
  - 【移除】移除防抖高亮工作项，不再有延迟高亮导致的突然抖动
  - 修改文件：Sources/View/带行号编辑器.swift

### v1.7.1
- 增强JS沙箱异常输出：
  - 【增强】语法错误/运行时异常现在输出详细位置信息：错误名称、错误消息、行号、列号
  - 【增强】自动输出出错行附近的代码上下文（前后各2行），用>>标记出错行，方便快速定位语法错误根源
  - 修改文件：Sources/Service/脚本沙箱服务.swift

### v1.7.0
- 两项修复与优化：
  - 【修复1】修复广告分析弹窗无效SF Symbol名称`wand.and.stars.slash`（iOS系统无此图标），统一使用`wand.and.stars`
  - 【优化2】脚本测试网络失败时自动兜底使用模拟响应体执行：之前网络请求失败直接终止测试，现在如果用户在「模拟响应体」中输入了JSON数据，自动使用模拟响应体继续执行脚本，实现离线测试能力。无模拟响应体时提示用户输入
  - 修改文件：Sources/View/广告分析弹窗.swift、Sources/ViewModel/脚本测试视图模型.swift

### v1.6.9
- 三项优化与修复：
  - 【修复1】彻底解决编辑器输入时剩余抖动：采用防抖高亮方案，输入时不重新设置attributedText，只更新绑定文本和刷新行号，停止输入0.3秒后才统一执行语法高亮。连续输入期间完全不触碰attributedText，从根本上消除UITextView重新布局导致的屏幕抖动
  - 【修复2】修复广告分析换域名后不管用的bug：根因是广告分析误用`请求体文本`作为响应体备选（请求体是发送给服务器的数据，不是服务器返回的响应体）。新增独立的`模拟响应体`输入框（默认折叠，橙色边框标识），与请求体完全分开。广告分析优先使用真实响应体，其次使用模拟响应体，不再误用请求体
  - 【优化3】广告分析按键反馈增强：不管有没有识别到广告字段都弹出弹窗反馈。空态时显示放大镜图标+详细操作指引（获取真实响应体/输入模拟响应体/检查JSON格式），一键生成按钮变灰禁用并显示"无广告字段可生成"。错误提示从阻断式改为提示式，与字段列表同时显示
  - 修改文件：Sources/View/带行号编辑器.swift、Sources/ViewModel/脚本测试视图模型.swift、Sources/View/脚本测试面板.swift、Sources/View/广告分析弹窗.swift
