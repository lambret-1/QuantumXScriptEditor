...
enum 代码补全服务 {
    ...
    static let 全部候选项: [代码补全项] = [
        // MARK: 常用代码片段（首字母联想优先显示，输入单个字母即可快速插入常用代码）
        // 输入 c 联想
        代码补全项(触发词: "c", 插入代码: "const ", 中文说明: "声明常量（const）", 分类: .常用片段),
        代码补全项(触发词: "const", 插入代码: "const ", 中文说明: "声明常量（const）", 分类: .常用片段),
        代码补全项(触发词: "console", 插入代码: "console.log(\"调试信息\");", 中文说明: "控制台输出调试信息（圈X标准写法）", 分类: .常用片段),
        代码补全项(触发词: "cl", 插入代码: "console.log(\"调试信息\");", 中文说明: "控制台输出调试信息（缩写cl）", 分类: .常用片段),
        代码补全项(触发词: "console.log", 插入代码: "console.log(\"变量值: \" + 变量);", 中文说明: "控制台输出变量值", 分类: .常用片段),
        // 输入 r 联想
        代码补全项(触发词: "r", 插入代码: "$response", 中文说明: "响应对象（$response）", 分类: .常用片段),
        代码补全项(触发词: "response", 插入代码: "$response", 中文说明: "响应对象（$response）", 分类: .常用片段),
        代码补全项(触发词: "rb", 插入代码: "$response.body", 中文说明: "响应体文本（缩写rb）", 分类: .常用片段),
        代码补全项(触发词: "rh", 插入代码: "$response.headers", 中文说明: "响应头字典（缩写rh）", 分类: .常用片段),
        代码补全项(触发词: "rs", 插入代码: "$response.statusCode", 中文说明: "响应状态码（缩写rs）", 分类: .常用片段),
        // 输入 d 联想
        代码补全项(触发词: "d", 插入代码: "$done();", 中文说明: "结束脚本执行（$done）", 分类: .常用片段),
        代码补全项(触发词: "done", 插入代码: "$done();", 中文说明: "结束脚本执行（$done）", 分类: .常用片段),
        代码补全项(触发词: "dr", 插入代码: "$done();\nreturn;", 中文说明: "结束脚本并return退出（缩写dr）", 分类: .常用片段),
        代码补全项(触发词: "db", 插入代码: "$done({ body: JSON.stringify(body) });", 中文说明: "返回修改后的JSON响应体（缩写db）", 分类: .常用片段),
        代码补全项(触发词: "de", 插入代码: "$done({});", 中文说明: "返回空响应（阻断请求，缩写de）", 分类: .常用片段),
        代码补全项(触发词: "do", 插入代码: "$done({ body: 原始响应体 });", 中文说明: "原样返回响应体（放行，缩写do）", 分类: .常用片段),
        // 输入 l 联想
        代码补全项(触发词: "l", 插入代码: "let ", 中文说明: "声明变量（let）", 分类: .常用片段),
        代码补全项(触发词: "let", 插入代码: "let ", 中文说明: "声明变量（let）", 分类: .常用片段),
        代码补全项(触发词: "lb", 插入代码: "let body = JSON.parse($response.body);", 中文说明: "解析响应体JSON为对象（缩写lb）", 分类: .常用片段),
        // 输入 v 联想
        代码补全项(触发词: "v", 插入代码: "var ", 中文说明: "声明变量（var）", 分类: .常用片段),
        代码补全项(触发词: "var", 插入代码: "var ", 中文说明: "声明变量（var）", 分类: .常用片段),
        代码补全项(触发词: "vb", 插入代码: "var body = $response.body;", 中文说明: "获取响应体文本（缩写vb）", 分类: .常用片段),
        // 输入 f 联想
        代码补全项(触发词: "f", 插入代码: "function ", 中文说明: "声明函数（function）", 分类: .常用片段),
        代码补全项(触发词: "function", 插入代码: "function ", 中文说明: "声明函数（function）", 分类: .常用片段),
        代码补全项(触发词: "fn", 插入代码: "function 函数名() {\n    // 函数体\n}", 中文说明: "完整函数模板（缩写fn）", 分类: .常用片段),
        代码补全项(触发词: "iife", 插入代码: "(function() {\n    // 立即执行函数，隔离变量\n})();", 中文说明: "IIFE立即执行函数（隔离变量避免污染全局）", 分类: .常用片段),
        // 输入 i 联想
        代码补全项(触发词: "i", 插入代码: "if ", 中文说明: "条件判断（if）", 分类: .常用片段),
        代码补全项(触发词: "if", 插入代码: "if ", 中文说明: "条件判断（if）", 分类: .常用片段),
        代码补全项(触发词: "ie", 插入代码: "if (typeof $response === 'undefined' || $response === null) {\n    $done({}); return;\n}", 中文说明: "检查$response是否存在（圈X标准写法，缩写ie）", 分类: .常用片段),
        代码补全项(触发词: "ib", 插入代码: "if (!原始响应体) {\n    $done({}); return;\n}", 中文说明: "检查响应体是否为空（缩写ib）", 分类: .常用片段),
        // 输入 t 联想
        代码补全项(触发词: "t", 插入代码: "try ", 中文说明: "异常捕获（try）", 分类: .常用片段),
        代码补全项(触发词: "try", 插入代码: "try ", 中文说明: "异常捕获（try）", 分类: .常用片段),
        代码补全项(触发词: "tc", 插入代码: "try {\n    // 可能出错的代码\n} catch (e) {\n    console.log(\"异常: \" + e);\n    $done({ body: 原始响应体 });\n}", 中文说明: "完整try-catch模板，异常时原样返回（缩写tc）", 分类: .常用片段),
        // 输入 j 联想
        代码补全项(触发词: "j", 插入代码: "JSON.parse(", 中文说明: "JSON解析（JSON.parse）", 分类: .常用片段),
        代码补全项(触发词: "jp", 插入代码: "JSON.parse(", 中文说明: "JSON解析（缩写jp）", 分类: .常用片段),
        代码补全项(触发词: "js", 插入代码: "JSON.stringify(", 中文说明: "JSON序列化（缩写js）", 分类: .常用片段),
        代码补全项(触发词: "json", 插入代码: "JSON.stringify(", 中文说明: "JSON序列化（JSON.stringify）", 分类: .常用片段),
        // 输入 n 联想
        代码补全项(触发词: "n", 插入代码: "$notify(", 中文说明: "弹出通知（$notify）", 分类: .常用片段),
        代码补全项(触发词: "notify", 插入代码: "$notify(", 中文说明: "弹出通知（$notify）", 分类: .常用片段),
        代码补全项(触发词: "nt", 插入代码: "$notify(\"标题\", \"副标题\", \"内容\");", 中文说明: "完整通知模板（缩写nt）", 分类: .常用片段),
        // 输入 p 联想
        代码补全项(触发词: "p", 插入代码: "$prefs.", 中文说明: "持久化存储（$prefs）", 分类: .常用片段),
        代码补全项(触发词: "prefs", 插入代码: "$prefs.", 中文说明: "持久化存储（$prefs）", 分类: .常用片段),
        代码补全项(触发词: "pw", 插入代码: "$prefs.setValueForKey(\"要保存的值\", \"存储键名\");", 中文说明: "写入持久化存储（缩写pw）", 分类: .常用片段),
        代码补全项(触发词: "pr", 插入代码: "$prefs.valueForKey(\"存储键名\");", 中文说明: "读取持久化存储（缩写pr）", 分类: .常用片段),
        // 输入 $ 联想
        代码补全项(触发词: "$", 插入代码: "$", 中文说明: "圈X全局对象前缀（$）", 分类: .常用片段),
        代码补全项(触发词: "$r", 插入代码: "$request", 中文说明: "请求对象（$request）", 分类: .常用片段),
        代码补全项(触发词: "$re", 插入代码: "$response", 中文说明: "响应对象（$response）", 分类: .常用片段),
        代码补全项(触发词: "$d", 插入代码: "$done();", 中文说明: "结束脚本（$done）", 分类: .常用片段),
        代码补全项(触发词: "$n", 插入代码: "$notify(", 中文说明: "弹出通知（$notify）", 分类: .常用片段),
        代码补全项(触发词: "$p", 插入代码: "$prefs.", 中文说明: "持久化存储（$prefs）", 分类: .常用片段),
        代码补全项(触发词: "$t", 插入代码: "$task.fetch(", 中文说明: "异步HTTP请求（$task.fetch）", 分类: .常用片段),
        // 输入 q 联想（圈X QuanX）
        代码补全项(触发词: "q", 插入代码: "// 圈X脚本", 中文说明: "圈X脚本注释（q）", 分类: .常用片段),
        代码补全项(触发词: "qx", 插入代码: "// 圈X脚本\n(function() {\n    // 脚本内容\n})();", 中文说明: "圈X脚本完整骨架（IIFE包裹，缩写qx）", 分类: .常用片段),
        // 输入 s 联想
        代码补全项(触发词: "s", 插入代码: "stringify", 中文说明: "JSON序列化（stringify）", 分类: .常用片段),
        代码补全项(触发词: "sb", 插入代码: "$done({ body: 原始响应体 });", 中文说明: "原样返回响应体（放行，缩写sb）", 分类: .常用片段),
        // 输入 e 联想
        代码补全项(触发词: "e", 插入代码: "else ", 中文说明: "否则（else）", 分类: .常用片段),
        代码补全项(触发词: "else", 插入代码: "else ", 中文说明: "否则（else）", 分类: .常用片段),
        // 输入 w 联想
        代码补全项(触发词: "w", 插入代码: "while ", 中文说明: "while循环（while）", 分类: .常用片段),
        代码补全项(触发词: "while", 插入代码: "while ", 中文说明: "while循环（while）", 分类: .常用片段),
        // 输入 o 联想
        代码补全项(触发词: "o", 插入代码: "Object.", 中文说明: "对象方法（Object）", 分类: .常用片段),
        代码补全项(触发词: "object", 插入代码: "Object.", 中文说明: "对象方法（Object）", 分类: .常用片段),
        // 输入 a 联想
        代码补全项(触发词: "a", 插入代码: "Array.", 中文说明: "数组方法（Array）", 分类: .常用片段),
        代码补全项(触发词: "array", 插入代码: "Array.", 中文说明: "数组方法（Array）", 分类: .常用片段),
        // 输入 m 联想
        代码补全项(触发词: "m", 插入代码: "Math.", 中文说明: "数学方法（Math）", 分类: .常用片段),
        代码补全项(触发词: "math", 插入代码: "Math.", 中文说明: "数学方法（Math）", 分类: .常用片段),
        // 输入 g 联想
        代码补全项(触发词: "g", 插入代码: "globalData", 中文说明: "全局广告配置对象（globalData）", 分类: .常用片段),
        代码补全项(触发词: "global", 插入代码: "globalData", 中文说明: "全局广告配置对象（globalData）", 分类: .常用片段),
    ...
    /// - Returns: 匹配的候选项列表，按优先级排序（常用片段优先，前缀匹配优先，短触发词优先）
    static func 过滤候选项(前缀: String) -> [代码补全项] {
        ...
        let 匹配项 = 全部候选项.filter { 项 in
            项.触发词.lowercased().contains(清洗前缀) ||
            项.中文说明.lowercased().contains(清洗前缀)
        }
        // 排序：常用片段优先 → 触发词以前缀开头的优先 → 触发词短的优先 → 触发词字母顺序
        return 匹配项.sorted { 项1, 项2 in
            // 1. 常用片段分类优先
            if 项1.分类 == .常用片段 && 项2.分类 != .常用片段 { return true }
            if 项1.分类 != .常用片段 && 项2.分类 == .常用片段 { return false }
            // 2. 触发词以前缀开头的优先（比包含匹配更精准）
            let 项1前缀匹配 = 项1.触发词.lowercased().hasPrefix(清洗前缀)
            let 项2前缀匹配 = 项2.触发词.lowercased().hasPrefix(清洗前缀)
            if 项1前缀匹配 && !项2前缀匹配 { return true }
            if !项1前缀匹配 && 项2前缀匹配 { return false }
            // 3. 触发词短的优先（单个字母触发词比长词更常用）
            if 项1.触发词.count != 项2.触发词.count {
                return 项1.触发词.count < 项2.触发词.count
            }
            // 4. 触发词字母顺序
            return 项1.触发词 < 项2.触发词
        }
    }
