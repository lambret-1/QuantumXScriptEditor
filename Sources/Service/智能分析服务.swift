    ...
    private static func 生成综合模板(结果: 分析结果) -> String {
        ...
        if !广告数组字段.isEmpty {
            ...
            for 字段 in 广告数组字段 {
                ...
                if 父级路径.isEmpty {
                    修改代码块 += "    if (Array.isArray(body.\(数组名))) { body.\(数组名) = body.\(数组名).filter(function(项) { try { return !是否为广告项(项); } catch (e) { return true; } }); console.log(\"   广告数组过滤: \(数组名) 已过滤\"); }\n"
                } else {
                    修改代码块 += "    if (body.\(父级路径) && Array.isArray(body.\(父级路径).\(数组名))) { body.\(父级路径).\(数组名) = body.\(父级路径).\(数组名).filter(function(项) { try { return !是否为广告项(项); } catch (e) { return true; } }); console.log(\"   广告数组过滤: \(字段.字段路径) 已过滤\"); }\n"
                }
            }
        }

        // --- 用户ID（敏感信息，保留原始值，仅注释说明）---
        let 用户ID字段列表 = 结果.用户核心字段.filter { $0.类型 == .用户ID }
        if !用户ID字段列表.isEmpty {
            修改代码块 += "    // ====== 用户ID（已识别，敏感信息保留原始值不修改）======\n"
            for 字段 in 用户ID字段列表 {
        ...
        if !头像字段列表.isEmpty {
            修改代码块 += "    // ====== 头像（设为默认头像URL）======\n"
            for 字段 in 头像字段列表 {
                修改代码块 += "    body.\(字段.字段路径) = \"https://default-avatar.com/default.png\";  // 设为默认头像\n"
            }
        }

        // --- 登录Token（敏感信息，保留原始值，仅注释说明）---
        let Token字段列表 = 结果.用户核心字段.filter { $0.类型 == .登录Token }
        if !Token字段列表.isEmpty {
            修改代码块 += "    // ====== 登录Token（已识别，敏感信息保留原始值不修改）======\n"
            for 字段 in Token字段列表 {
